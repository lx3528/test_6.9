...
%% find huchang
t=0.1;index=60;zui_length=20;beta_long=linspace(alpha/2+0.00001,pi/2,index)';
inp=[1];
%% two displacement
%0 point
...
for j=1:size(inp,2)
    ...
    moduan=alpha;
    beta1=[pi/1024:0.01:moduan/2]';beta=[beta1;moduan/2]; num=size(beta,1);
    index2=num;
    ...
    xpp1=zhili_displace1(:,2).*cos(pi/2-beta)-zhili_displace1(:,3).*sin(pi/2-beta)+x_start;
    ypp1=zhili_displace1(:,2).*sin(pi/2-beta)+zhili_displace1(:,3).*cos(pi/2-beta)+y_start;
    plot(xpp1,ypp1)
    hold on;
    
    k=tan(pi-alpha/2);
    x_zhuanzhuan=[0;flipud(x_zhuan2);x_zhuan3;xpp1];
    y_zhuanzhuan=[0;flipud(y_zhuan2);y_zhuan3;ypp1];
   ...
   huchang=rao_displace(j,1)+zhili_displace1(num,1);
end